function ExcelOutputMatrix = FormattedOutput_test(ExcelOutputMatrix, phi, mix, species)
    R0 = 8.3144598;
    ...
    %     if (str{1}.Xi(j) > 0)||(any(strcmp(NameSpecies{j},{'CO2','CO','H2O','H2','O2','N2'})))
            ...
            ExcelOutputMatrix{1,12+SpeciesCounter} = [composition_variable,species{j}];
    ...
    for i=length(phi):-1:1
        ...
        ExcelOutputMatrix{i+2,2} = mix{i}.T;
        ExcelOutputMatrix{i+2,3} = mix{i}.p;
        ExcelOutputMatrix{i+2,4} = mix{i}.W;
        ExcelOutputMatrix{i+2,5} = R0/mix{i}.W/1e-3;
        ExcelOutputMatrix{i+2,6} = mix{i}.rho;
        ExcelOutputMatrix{i+2,7} = mix{i}.h/mix{i}.mi;
        ExcelOutputMatrix{i+2,8} = mix{i}.DhT/mix{i}.mi;
        ExcelOutputMatrix{i+2,9} = mix{i}.cP/mix{i}.mi*1e-3;
        ...
        ExcelOutputMatrix{i+2,10} = mix{i}.gamma;
        ExcelOutputMatrix{i+2,11} = mix{i}.S;
        if isfield(mix{i},'u')
            ExcelOutputMatrix{i+2,12} = mix{i}.u;
        end
        SpeciesCounter = 0;
        for j = 1:numel(species)
    ...
    %             eval(['ExcelOutputMatrixRow{1,10+SpeciesCounter} = ',str{i}.Xi(j),';']);
                ExcelOutputMatrix{i+2,12+SpeciesCounter} = mix{i}.Xi(j);
    %         end
        end
    %     ExcelOutputMatrix = [ExcelOutputMatrix; ExcelOutputMatrix];
