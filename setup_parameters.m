function parameters = setup_parameters()
    parameters.mass_per_satellite = 250; %kg
    ...
    parameters.sat_prod_learning_rate = 0.65;

    parameters.launcher_refurb_time_percent = [0.3, 0.3]; %Percentage of total launcher prod time; Need to set stage 2 refurb time. shuttle avg, but space x ar 27 to 21 days. source: https://ark-invest.com/newsletters/issue-335/
    parameters.launcher_prod_time_bins = [1.5,3,4.5; 0.5,3, 6]; %Small based on electron, medium based on falcon9, large is a random number for now
    ...
    parameters.init_sat_prod_time = 20; %days, lets try this value and see if the learning rate drops fast enough to realistic values  
    ...
    parameters.max_num_engines = 200;
